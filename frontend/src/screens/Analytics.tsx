...
import { velocityStats, moa, mil, vel, velSd, group } from '../lib/stats';
...
export default function Analytics() {
  ...
  const rows = useMemo(() => strings.map((f) => {
    ...
    return { f, load, n: st.n, avgRaw: st.avg, sdRaw: st.sd, esRaw: st.es, avg: st.avg && st.avg * k, sd: st.sd && st.sd * k, es: st.es && st.es * k, moa: g, rifleId: sessions.find((x) => x.id === f.range_session_id)?.rifle_id };
  }).filter((r) => r.load && (!rifle || r.rifleId === rifle)), [strings, shots, loads, rifle, sessions, s]);
  ...
  return (
    <div className="stack">
      ...
      <div className="grid2">
        ...
        <select value={y} onChange={(e) => setY(e.target.value as any)}>{Object.entries(METRICS).map(([k, v]) => <option key={k} value={k}>{v} ({k === 'moa' ? s.groupUnit.toUpperCase() : s.velocityUnit === 'mps' ? 'm/s' : 'fps'})</option>)}</select>
      ...
      <table className="table">
        ...
        <tbody>{rows.map((r) => <tr key={r.f.id}><td>{r.load?.name}</td><td>{r.n}</td><td>{vel(r.avgRaw, s)}</td><td>{velSd(r.sdRaw, s)}</td><td>{vel(r.esRaw, s)}</td><td>{group(r.f.group_size_inches, r.f.target_distance_yards, s)}</td></tr>)}</tbody>
      </table>
